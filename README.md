...
- 로고, 학회명, 상단 메뉴(header)는 이제 각 페이지에 따로 들어있지 않고 `_includes/header.html`
  파일 하나로 공통 관리됩니다. 메뉴 항목을 추가/삭제하거나 로고를 바꾸고 싶으면 이 파일 하나만
  수정하면 모든 페이지에 자동으로 반영됩니다.
- 하단 저작권 표시(footer)도 마찬가지로 `_includes/footer.html` 파일 하나로 관리됩니다.
- 이 방식은 GitHub Pages가 기본으로 제공하는 Jekyll 빌드 기능(`{% include %}`)을 사용합니다.
  별도 설정이나 빌드 명령 없이, 지금처럼 그냥 push만 하면 GitHub가 자동으로 조립해서 배포합니다.
  단, 각 `.html` 페이지 맨 위에 있는 빈 `---` `---` 두 줄(front matter)은 Jekyll이 그 파일을
  처리 대상으로 인식하게 하는 표시이니 지우면 안 됩니다.
- 메뉴에서 현재 페이지를 강조하는 "active" 표시(황금색 배경)는 파일명을 기준으로
  `_includes/header.html` 안에서 자동으로 계산되므로, 새 페이지를 추가할 때도 따로
  active 클래스를 손으로 넣을 필요가 없습니다.
